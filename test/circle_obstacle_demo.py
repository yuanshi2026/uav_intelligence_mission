...
# 2.2 起飞和任务参数
# ------------------------------
DEFAULT_TAKEOFF_ALT = 1.0             # 起飞高度，单位 m
...
# 2.3 圆形绕障参数
# ------------------------------
DEFAULT_RADIUS = 1.2                  # 绕圆半径，单位 m；实飞首次建议 >= 1.2
DEFAULT_LOOPS = 3.0                   # 绕圆圈数；首次实飞建议 1 圈
DEFAULT_CIRCLE_SPEED = 0.8           # 绕圆切向速度，单位 m/s；首次实飞建议 0.3~0.4
...
class CircleObstacleGroundStationNode:
    ...
    def send_status(self, addr=None):
        ...
        text = (
            ...
            "mission_active=%s, last_command=%s"
            % (
    ...
    def move_to(self, x, y, z, yaw=None, speed=None, hold_time=0.5):
        ...
        for i in range(steps + 1):
            if rospy.is_shutdown() or self.is_land_requested():
                return False
    ...
    def fly_circle(self):
        ...
        rospy.loginfo("开始绕圆：半径=%.2f, 圈数=%.2f, 速度=%.2f, 总时间=%.2f 秒",
                      self.radius, self.loops, self.circle_speed, total_time)
        self.send_udp_message(self.last_ground_addr, "STATUS：开始绕圆，预计 %.1f 秒" % total_time)
